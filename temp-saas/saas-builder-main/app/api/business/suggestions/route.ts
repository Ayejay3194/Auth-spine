...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${spineUrl}/assistant/suggestions`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`Business Spine suggestions failed: ${response.status}`)
    }
    ...
    console.error('Business Spine suggestions error:', error)
    return NextResponse.json(
