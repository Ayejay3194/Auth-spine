...
export function extractId(text: string, prefix: string): string | null {
...
export function extractDateTime(text: string, nowISO: string): { dateISO?: string; time?: string; dateTimeISO?: string } {
  ...
  if (tm) {
    ...
    if (tm24) time = `${tm24[1].padStart(2,"0")}:${tm24[2]}`;
  }
  ...
  if (!date) {
    ...
    if (m) {
      const mm = MONTHS[m[1]];
      if (mm) date = new Date(now.getFullYear(), mm - 1, Number(m[2]));
    }
  }

  // relative
  if (!date) {
    if (t.includes("today")) date = startOfDay(now);
    else if (t.includes("tomorrow")) date = startOfDay(addDays(now, 1));
...
export function requireFields(entities: Record<string, unknown>, required: string[]): string[] {
