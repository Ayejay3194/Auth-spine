...
export class TrustVelocitySystem implements Module {
  meta = { id: "37.trust-velocity", name: "Trust Velocity System", version: "1.0.0" };
  ...
  async init(ctx: ModuleContext) {
    ctx.events.subscribe("booking.requested", async (e) => {
      ...
      const input = await this.buildInput(ctx, userId, professionalId);
      const trust = this.scoreTrust(input);
      await ctx.store.set(`trust:${userId}:${professionalId}`, trust);
      ...
      await ctx.events.publish({
        ...
        at: ctx.now(),
        subject: { userId, professionalId } as any,
        payload: { depositRequired, trust },
      });
    });
  }

  private async buildInput(ctx: ModuleContext, userId: ID, professionalId: ID): Promise<TrustInput> {
    ...
    const priorBookings = (await ctx.store.get<number>(`user:prior_bookings:${userId}:${professionalId}`)) ?? 0;
    const noShowHistory = (await ctx.store.get<number>(`user:no_show_count:${userId}`)) ?? 0;
    return { userId, professionalId, profileCompleteness, responseLatencyMinutes, priorBookings, noShowHistory };
  }
  ...
  private scoreTrust(input: TrustInput): ScoreResult {
    ...
    return { key: "trust_score", value, level: value < 0.35 ? "high" : value < 0.55 ? "medium" : "low", signals: { completion, latency, history, noShowRisk } };
  }
}
