...
import { AssistantContext } from "../core/types.js";

const orch = createDefaultOrchestrator();

// Test context
const ownerContext: AssistantContext = {
...
const adminContext: AssistantContext = {
...
const staffContext: AssistantContext = {
...
function assertOk(result: { ok?: boolean; final?: { ok: boolean } }, message?: string) {
...
const tests = [
  test("Booking: Create booking", async () => {
    const result = await orch.handle(
      ...
      { ...ownerContext, nowISO: new Date().toISOString() }
    );
  ...
  test("CRM: Add note to client", async () => {
    const result = await orch.handle(
      "add note for alex@example.com: prefers morning appointments",
      { ...ownerContext, nowISO: new Date().toISOString() }
    );
  ...
  test("Payments: Create invoice requires confirmation", async () => {
    ...
    assertEqual(
      result.final?.payload && typeof result.final.payload === "object" && "confirmToken" in result.final.payload,
      true,
  ...
  test("Marketing: Create promo code", async () => {
    const result = await orch.handle(
      "create promo code GLOWUP 15% expires next friday 5pm",
      { ...ownerContext, nowISO: new Date().toISOString() }
    );
  ...
  test("Diagnostics: Admin can run diagnostics", async () => {
    const result = await orch.handle(
      "run diagnostics",
      { ...adminContext, nowISO: new Date().toISOString() }
    );
  ...
  test("Diagnostics: Staff cannot run diagnostics", async () => {
    const result = await orch.handle(
      "run diagnostics",
      { ...staffContext, nowISO: new Date().toISOString() }
    );
  ...
  test("Intent Detection: Handles unknown commands", async () => {
    const result = await orch.handle(
      "do something completely random xyz123",
      { ...ownerContext, nowISO: new Date().toISOString() }
    );
