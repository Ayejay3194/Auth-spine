...
export type AstroRecord = {
  id?: string;
  book?: string;
  topic?: "sun" | "moon" | "mercury" | "venus" | "mars" | "jupiter" | "saturn" | "uranus" | "neptune" | "pluto" | "asc" | "mc";
  sign?: string;
  house?: number;
  text: string;
  keywords?: string[];
 ...
 */
function queryAstrologyDB(query: { planet?: string; sign?: string; house?: number }): AstroRecord[] {
  const results: AstroRecord[] = [];

  for (const record of ASTRO_DATABASE) {
    let score = 0;

    if (query.planet && record.topic === query.planet) score += 10;
    if (query.sign && record.sign === query.sign) score += 10;
    if (query.house && record.house === query.house) score += 10;
  ...
  results.sort((a, b) => {
    ...
    if (query.planet) {
      if (a.topic === query.planet) scoreA += 10;
      if (b.topic === query.planet) scoreB += 10;
    }
    if (query.sign) {
      if (a.sign === query.sign) scoreA += 10;
      if (b.sign === query.sign) scoreB += 10;
    }
...
export async function readAstrologyPlacement(args: {
  ...
  try {
    ...
    const records = queryAstrologyDB({
      planet: planet as string | undefined,
      sign: sign as string | undefined,
      house: house as number | undefined,
    });
