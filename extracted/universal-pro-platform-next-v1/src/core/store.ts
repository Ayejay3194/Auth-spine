...
export class InMemoryStore implements KVStore {
  ...
  private purge(key: string) { const e = this.m.get(key); if (!e) return; if (e.expiresAt && Date.now() > e.expiresAt) this.m.delete(key); }
  async get<T>(key: string): Promise<T | null> { this.purge(key); const e = this.m.get(key); return e ? (e.value as T) : null; }
  ...
  async scan(prefix: string, limit = 100): Promise<string[]> {
    const out: string[] = [];
    for (const k of this.m.keys()) { if (k.startsWith(prefix)) out.push(k); if (out.length >= limit) break; }
    return out;
  }
}
