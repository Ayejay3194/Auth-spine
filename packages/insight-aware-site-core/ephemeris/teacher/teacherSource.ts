...
/**
 * Teacher source: High-precision VSOP87 ephemeris
 * Reference implementation for teacher-student learning
 */
export class TeacherSource implements EphemerisEngine {
  name = "TeacherSource(VSOP87-Reference)";
  ...
  private computeMoon(t: number): EphemerisOutput {
    ...
    const lon = wrap2pi(
      L + 
      0.10976 * Math.sin(D) +
      0.02224 * Math.sin(M - D) +
      0.01149 * Math.sin(M) +
      0.00373 * Math.sin(2 * D) +
      0.00196 * Math.sin(2 * M - D)
    );
    
    const lat = (
      0.08950 * Math.sin(D + M) +
      0.00490 * Math.sin(2 * D - M) +
      0.00232 * Math.sin(2 * D)
    );
