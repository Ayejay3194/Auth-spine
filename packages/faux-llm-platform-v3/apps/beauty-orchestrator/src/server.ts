...
export function makeOrchestrator(appId: string, allowedTools: string[]) {
  ...
  async function chat(sessionId: string, userText: string, goalText?: string): Promise<Json> {
    let toolCalls = 0;
    ...
    const scored = await rag.retrieveScored!({ query: userText, k: 6 });
    ...
    const req = {
      messages: [
        ...
        { role: "user", content: `MEMORY:\n${(await memory.query({ contains: userText, limit: 6 })).map(n => `- ${n.key}: ${n.value} (c=${n.confidence.toFixed(2)})`).join("\n") }\n\nCONTEXT:\n${context}\n\nUSER:\n${userText}` }
      ],
...
export function startServer(appId: string, allowedTools: string[], port: number) {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/chat") {
      ...
      const out = await orch.chat(sessionId, text, goal);
      res.writeHead(200, { "Content-Type": "application/json" });
      return res.end(JSON.stringify(out));
    }
...
startServer("BeautyBooking", ["calc", "echo"], 5053);
