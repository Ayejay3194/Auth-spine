 ...
 */
import { BusinessSpineApi } from "../api/server.js";
...
const tests = [
    ...
    test("API: Health check", async () => {
        ...
        assertEqual(result.data && typeof result.data === "object" && "status" in result.data, true, "Should return status");
    }),
    ...
    test("API: Detect intents", async () => {
        ...
        assertEqual(result.data && typeof result.data === "object" && "intents" in result.data, true, "Should return intents array");
    }),
    test("API: Handle request with confirmation token", async () => {
        ...
        if (result1.data &&
            ...
            const token = result1.data.final.payload.confirmToken;
